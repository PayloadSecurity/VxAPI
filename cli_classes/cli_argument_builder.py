...
class CliArgumentBuilder:
    ...
    def add_file_with_hash_list_arg(self, allowed_hashes):
        self.parser.add_argument('hashes-file', type=argparse.FileType('r'), help='Path to file containing list of sample hashes (allowed: {})'.format(allowed_hashes.join(', ')))

    def add_file_with_ids_arg(self, allowed_ids):
        self.parser.add_argument('mixed-ids-file', type=argparse.FileType('r'), help='Path to file containing list of ids (allowed: {}'.format(allowed_ids.join(', ')))
    ...
    def add_search_term_filename_opt(self):
        self.parser.add_argument('--filename', '-fn', type=str, help='Filename e.g. invoice.exe')

    def add_search_term_filetype_opt(self):
        self.parser.add_argument('--filetype', '-ft', type=str, help='Filetype e.g. docx')

    def add_search_term_filetype_desc_opt(self):
        self.parser.add_argument('--filetype-desc', '-ftd', type=str, help='Filetype description e.g. PE32 executable')

    def add_search_term_env_id_opt(self):
        self.parser.add_argument('--env-id', '-ei', type=int, help='Environment Id')

    def add_search_term_country_opt(self):
        self.parser.add_argument('--country', '-co', type=str, help='Country (3 digit ISO) e.g. swe')

    def add_search_term_verdict_opt(self):
        self.parser.add_argument('--verdict', '-vs', type=str, help='Verdict', choices={1: 'whitelisted', 2: 'no verdict', 3: 'no specific threat', 4: 'suspicious', 5: 'malicious'})

    def add_search_term_av_detect_opt(self):
        self.parser.add_argument('--av-detect', '-ad', type=str, help='AV Multiscan range e.g. 50-70 (min 0, max 100)')

    def add_search_term_vx_family_opt(self):
        self.parser.add_argument('--vx-family', '-vf', type=str, help='AV Family Substring e.g. nemucod')

    def add_search_term_tag_opt(self):
        self.parser.add_argument('--tag', '-t', type=str, help='Hashtag e.g. ransomware')

    def add_search_term_port_opt(self):
        self.parser.add_argument('--port', '-p', type=str, help='Port e.g. 8080')

    def add_search_term_host_opt(self):
        self.parser.add_argument('--host', '-h', type=str, help='Host e.g. 192.168.0.1')

    def add_search_term_domain_opt(self):
        self.parser.add_argument('--domain', '-d', type=str, help='Domain e.g. checkip.dyndns.org')

    def add_search_term_url_opt(self):
        self.parser.add_argument('--url', '-u', type=str, help='HTTP Request Substring e.g. google')

    def add_search_term_similar_to_opt(self):
        self.parser.add_argument('--similar-to', '-st', type=str, help='Similar Samples e.g. <sha266>')

    def add_search_term_context_opt(self):
        self.parser.add_argument('--context', '-cn', type=str, help='Sample Context e.g. <sha266>')

    def add_search_term_imp_hash_opt(self):
        self.parser.add_argument('--imphash', '-ih', type=str)

    def add_search_term_ssdeep_opt(self):
        self.parser.add_argument('--ssdeep', '-sd', type=str)

    def add_search_term_authentihash_opt(self):
        self.parser.add_argument('--authentihash', '-ah', type=str)
