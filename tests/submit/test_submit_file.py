...
class TestSubmitFile(BaseTest):
    ...
    def init_request_scenario(self):
        os.environ['VX_TEST_SCENARIO'] = 'submit.submit_file'

    def test_base_query(self, run_command):
        self.init_request_scenario()

        run_command(self.get_action_name(), 'tests/_data/hashes', '5')
        self.see_response(self.expected_response)

    def test_verbose_query(self, run_command):
        self.init_request_scenario()

        run_command(self.get_action_name(), 'tests/_data/hashes', '5', '-v')
        self.see_headers()
        self.see_response(self.expected_response)
